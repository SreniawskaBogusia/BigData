...
val filePath = "dbfs:/FileStore/tables/online_retail_dataset.delta"
df.write.format("delta").partitionBy("Country").mode("overwrite").save(filePath)
val dfDelta = spark.read
  .format("delta").load(filePath).repartition(400)
...
// TODO
val filePath = "dbfs:/FileStore/tables/online_retail_dataset.delta/Country=Sweden"
val plikiPrzed = dbutils.fs.ls(filePath).length
...
// TODO
val plikiPo = dbutils.fs.ls(filePath).length
